...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <button data-testid="send" onClick={() => chat.sendMessage("test")}>
        Send
      </button>
    </div>
  );
...
describe("ChatContext", () => {
  ...
  test("tracks anonymous work when no project ID", async () => {
    const mockMessages = [{ id: "1", role: "user" as const, parts: [{ type: "text" as const, text: "Hello" }] }];
  ...
  test("passes through AI chat status", () => {
    (useAIChat as any).mockReturnValue({
      ...mockUseAIChat,
      status: "streaming",
    });
